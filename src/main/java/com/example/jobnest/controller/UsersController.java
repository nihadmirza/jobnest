...
public class UsersController {
    ...
    public String userRegistration(@Valid Users users, Model model) {
        try {
            ...
            return "redirect:/dashboard"; //prg pattern
        } catch (CustomException e) {
            model.addAttribute("error", e.getMessage());

            List<UsersType> usersTypes = usersTypeService.getAll();
            model.addAttribute("getAllTypes", usersTypes);
            model.addAttribute("user", users); // formun silinmemesi ucun melumatlari geri qaytaririq
            return "register";
        } catch (Exception e) {
            // Diger xetalar ucun
            model.addAttribute("error", "Sistemdə xəta baş verdi. Zəhmət olmasa yenidən cəhd edin.");
            return "register";
        }
    }
}
